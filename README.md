...
+ `SELECT sum("vcores") FROM ( select last(vcoreSeconds) AS "vcores", "appname" from "yarn_apps" WHERE $timeFilter AND "appname" =~ /^miremr.*$/ GROUP BY "id",time(1d))  GROUP BY "appname", time($__interval)`
+ ``
